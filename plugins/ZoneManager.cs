...
{
    [Info("ZoneManager", "Reneb / Nogrod / k1lly0u", "3.0.5")]
    ...
    public class ZoneManager : RustPlugin
    {
        ...
        private void OnEntityKill(BaseEntity baseEntity)
        {
            ...
            if (zonedEntities.TryGetValue(baseEntity.net.ID, out entityZones))
            {
                ...
                {
                    entityZones.Zones.ElementAt(i)?.OnEntityExitZone(baseEntity, false);
                }
        ...
        private void OnPlayerSleep(BasePlayer player)
        {
            ...
            // Manually update the zones a player is in. Sleeping players don't trigger OnTriggerEnter or OnTriggerExit
            player.Invoke(()=> UpdatePlayerZones(player), 1f);
        ...
        private void EjectPlayer(BasePlayer player, Zone zone)
        {
            ...
            if (position == Vector3.zero)
            {
                ...
                position = zone.transform.position + (player.transform.position - zone.transform.position).normalized * (distance + 5f);
        ...
        private void AttractPlayer(BasePlayer player, Zone zone)
        {
            ...
            SendMessage(player, msg("attract", player.UserIDString));
        }
        ...
        public class Zone : MonoBehaviour
        {
            ...
            //    if (definition.Size != Vector3.zero)
            //        Vis.Entities<BasePlayer>(new OBB(transform.position, transform.rotation, bounds), list, PLAYER_MASK);
            //    else Vis.Entities<BasePlayer>(transform.position, definition.Radius, list, PLAYER_MASK);
            ...
            public void OnEntityExitZone(BaseEntity baseEntity, bool resetDecay)
            {
            ...
            public bool CanEnterZone(BasePlayer player) => HasPermission(player) || !CanLeaveZone(player);
        ...
        private object canRedeemKit(BasePlayer player) => CanRedeemKit(player);
                ...
                switch (args[i].ToLower())
                {
                    ...
                    case "rotation":
                        ...
                        if (rotation is float)
                            zone.definition.Rotation = Quaternion.AngleAxis((float)rotation, Vector3.up).eulerAngles;
